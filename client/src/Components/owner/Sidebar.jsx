  ...
  return (
    ...
    md:max-w-60 w-full border-r border-borderColor text-sm'>
        <div className='group relative'>
            <label htmlFor="image">
                <img src={image ? URL.createObjectURL(image):user?.image  || assets.avatar} alt=""  
        ...
        {image && (
            <button className='absolute top-0 right-0 flex p-2 gap-1
             bg-primary/10 text-primqary cursor-pointer'  onClick={updateImage}>Save 
        ...
        <div className='w-full'>
            {ownerMenuLinks.map((link,index)=>(
                ...
                ${link.path === location.pathname ? 'bg-primary/10 text-primary':'text-gray-600'}`}>
                    <img  className="w-5 h-5"  src={link.path === location.pathname ?link.coloredIcon :link.icon} alt="car-icon" />
